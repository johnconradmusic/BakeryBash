...
namespace Monocle
{
	public static class Calc
	{
		...
		{
			if (choices.Count == 0) return default(T);
		...
		{
			return random.Chance((float)chance / 100);
		}
		public static bool Chance(this Random random, float chance)
		{
			return random.NextFloat() < chance;
		}
		public static float MinusOneToOne(this Random random)
		{
			return random.Range(-100, 101) / 100f;
		}
		...
		public static int Approach(int val, int target, int maxMove) => Approach((int)val, (int)target, (int)maxMove);
		...
		public static XmlDocument LoadContentXML(string filename)
		{
			...
			xml.Load(TitleContainer.OpenStream(ConvertPath(Path.Combine(Engine.Instance.Content.RootDirectory, filename))));
			return xml;
		}
