...
namespace BakeryBash
{
	...
	{
		public const string SaveDataTitle = "Bakery Bash Save Data";
		private const string SavePath = "Saves";
		private const string BackupPath = "Backups";
		private const string Extension = ".bb";
		...
		private static string GetHandle(string name) => Path.Combine("Saves", name + ".bb");

		private static string GetBackupHandle(string name) => Path.Combine("Backups", name + ".bb");
		...
		public static bool TryLoad()
		{
			if (UserIO.Open(UserIO.Mode.Read))
			{
				...
				{
					SaveData.StartNew(new SaveData());
					UserIO.Close();
				}
		...
		public static void SaveHandler(bool file, bool settings)
		{
			if (UserIO.Saving)
				return;
			UserIO.Saving = true;
			BakeryBash.SaveRoutine = new Coroutine(UserIO.SaveRoutine(file, settings));
		}
		...
		private static IEnumerator SaveRoutine(bool file, bool settings)
		{
			...
			UserIO.Saving = false;
			BakeryBash.SaveRoutine = (Coroutine)null;
