...
namespace BakeryBash
{
	public class TextMenu : Entity
	{
		...
		public float ItemSpacing = 6f;
		public float MinWidth;
		public float Alpha = 1f;
		public Color HighlightColor = Color.White;
		public static readonly Color HighlightColorA = Colors.WorldTitle; //Calc.HexToColor("84FF54");
		public static readonly Color HighlightColorB = Colors.WorldTitleOutline; //Calc.HexToColor("FCFF59");
		public Action OnESC;
		...
		{
			get => this.items.Count <= 0 || this.Selection < 0 ? null : this.items[this.Selection];
		...
		public TextMenu()
		{
			...
			this.Position = new Vector2(Engine.Width, Engine.Height) / 2f;
		...
		public TextMenu Add(TextMenu.Item item)
		{
			this.items.Add(item);
			item.Container = this;
			this.Add(item.ValueWiggler = Wiggler.Create(0.25f, 3f));
			this.Add(item.SelectWiggler = Wiggler.Create(0.25f, 3f));
		...
		public Rectangle GetTouchTargetOf(TextMenu.Item item)
		{
		...
		public float GetYOffsetOf(TextMenu.Item item)
		{
			...
			foreach (TextMenu.Item obj in this.items)
			{
				if (item.Visible)
					if (obj == item)
						break;
				result += obj.Height() + this.ItemSpacing;
			}
			return result;// - item.Height() * 0.5f - this.ItemSpacing;
		}
		...
		public void CloseAndRun(IEnumerator routine, Action onClose)
		{
			this.Focused = false;
			this.Visible = false;
			this.Add(new Coroutine(this.CloseAndRunRoutine(routine, onClose)));
		}
		...
		public override void Update()
		{
			...
			Vector2 justifiedPosition = this.Position - this.Justify * new Vector2(this.Width, this.Height);

			foreach (TextMenu.Item obj in this.items)
			{
				var height = obj.Height();
				obj.Position = justifiedPosition + new Vector2(Width / 2, GetYOffsetOf(obj));
				//justifiedPosition.Y += height + this.ItemSpacing;
			}
			...
			if (this.Focused)
			{
				...
				{
					var pos = MInput.Mouse.Position;

					foreach (Item item in items)
					{
						if (GetTouchTargetOf(item).Contains(pos.X, pos.Y))
						{
							int selection = this.Selection;
				...
				if (MInput.Touch.Tapped)
				{
					...
					foreach (Item item in items)
					{
						var touchTarget = GetTouchTargetOf(item);
						Calc.Log(touchTarget);
						if (touchTarget.Contains(pos.X, pos.Y))
						{
							int selection = this.Selection;
							Selection = IndexOf(item);
							Current = item;
			...
			if (!this.AutoScroll)
				return;
			if ((double)this.Height > (double)this.ScrollableMinSize)
				this.Position.Y += (float)(((double)this.ScrollTargetY - Position.Y) * (1.0 - Math.Pow(0.0099999997764825821, (double)Engine.RawDeltaTime)));
			else
				this.Position.Y = Y;
		}
		...
		public float ScrollTargetY
		{
			get
			{
				float min = Engine.Height - 150 - this.Height * this.Justify.Y;
				float max = (float)(150.0 + (double)this.Height * Justify.Y);
				return Calc.Clamp(Engine.Height / 2 + this.Height * this.Justify.Y - this.GetYOffsetOf(this.Current), min, max);
			}
		}
		...
		public override void Render()
		{
			...
			foreach (TextMenu.Item obj in this.items)
			{
				if (obj.Visible)
				{
					...
					if (!obj.AboveAll)
						obj.Render(justifiedPosition + new Vector2(0.0f, (float)((double)height * 0.5 + (double)obj.SelectWiggler.Value * 8.0)), this.Focused && this.Current == obj);
					else
						flag = true;
					justifiedPosition.Y += height + this.ItemSpacing;
				}
				//Draw.HollowRect(GetTouchTargetOf(obj), Color.Yellow);
			}
			if (!flag)
				return;
			foreach (TextMenu.Item obj in this.items)
			{
				if (obj.Visible)
				{
					...
					if (obj.AboveAll)
						obj.Render(justifiedPosition + new Vector2(0.0f, (float)((double)num * 0.5 + (double)obj.SelectWiggler.Value * 8.0)), this.Focused && this.Current == obj);
					justifiedPosition.Y += num + this.ItemSpacing;
				}
			}
		}
		...
		public class Header : TextMenu.Item
		{
			...
			public override void Render(Vector2 position, bool highlighted)
			{
				...
				ActiveFont.DrawEdgeOutline(this.Title, position + new Vector2(this.Container.Width * 0.5f, 0.0f), new Vector2(0.5f, 0.5f), Vector2.One * 2f, Colors.TitleColor * alpha, 4f, Colors.WorldTitleOutline * alpha, 2f, strokeColor);
			}
		}
		...
		public class SubHeader : TextMenu.Item
		{
			...
			public override float Height() => (float)((this.Title.Length > 0 ? (double)ActiveFont.LineHeight * 0.60000002384185791 : 0.0) + (this.TopPadding ? 54.0 : 0.0));
			...
			public override void Render(Vector2 position, bool highlighted)
			{
				...
				ActiveFont.DrawOutline(40, this.Title, position + (this.Container.InnerContent == TextMenu.InnerContentMode.TwoColumn ? new Vector2(0.0f, y) : new Vector2(this.Container.Width * 0.5f, y)), new Vector2(this.Container.InnerContent == TextMenu.InnerContentMode.TwoColumn ? 0.0f : 0.5f, 0.5f), Vector2.One * 0.6f, Color.Gray * alpha, 2f, strokeColor);
			}
		}
		...
		public class Option<T> : TextMenu.Item
		{
			...
			public override void Render(Vector2 position, bool highlighted)
			{
				...
				ActiveFont.DrawOutline(30, this.Label, position, new Vector2(0.0f, 0.5f), Vector2.One, color1, 2f, strokeColor);
				if (this.Values.Count <= 0)
					return;
				float num = this.RightWidth();
				ActiveFont.DrawOutline(30, this.Values[this.Index].Item1, position + new Vector2((float)((double)this.Container.Width - (double)num * 0.5 + lastDir * (double)this.ValueWiggler.Value * 8.0), 0.0f), new Vector2(0.5f, 0.5f), Vector2.One * 0.8f, color1, 2f, strokeColor);
				...
				ActiveFont.DrawOutline(30, "<", position + new Vector2((float)((double)this.Container.Width - (double)num + 40.0 + (this.lastDir < 0 ? -(double)this.ValueWiggler.Value * 8.0 : 0.0)), 0.0f) - (flag1 ? vector2 : Vector2.Zero), new Vector2(0.5f, 0.5f), Vector2.One, color2, 2f, strokeColor);
				bool flag2 = this.Index < this.Values.Count - 1;
				Color color3 = flag2 ? color1 : Color.DarkSlateGray * alpha;
				ActiveFont.DrawOutline(30, ">", position + new Vector2((float)((double)this.Container.Width - 40.0 + (this.lastDir > 0 ? (double)this.ValueWiggler.Value * 8.0 : 0.0)), 0.0f) + (flag2 ? vector2 : Vector2.Zero), new Vector2(0.5f, 0.5f), Vector2.One, color3, 2f, strokeColor);
			}
		}
		...
		public class Button : TextMenu.Item
		{
			...
			public override float LeftWidth() => Fonts.ComicGecko.Get(50).Measure(this.Label).X;

			public override float Height() => Fonts.ComicGecko.Get(50).LineHeight;
			...
			public override void Render(Vector2 position, bool highlighted)
			{
				...
				Fonts.ComicGecko.DrawOutline(50, this.Label, position + (flag ? Vector2.Zero : new Vector2(this.Container.Width * 0.5f, 0.0f)), !flag || this.AlwaysCenter ? new Vector2(0.5f, 0.5f) : new Vector2(0.0f, 0.5f), Vector2.One, color, 2f, strokeColor);
			}
		}
	}
}
